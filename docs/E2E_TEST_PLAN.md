...
| B6 | Removed secondary-account event stays backward-readable ✅ | Covered by the legacy-event unit test in `scenario-progress.test.ts` | Old `home.savings.open` recordings remain `error`; the current home UI cannot emit the event |
