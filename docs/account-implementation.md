...
CSS implements the 39-second loop using the live node positions, rotations and relative scales. The original local 2.8 KB SVG carries the Figma blob shape, blur, color and grain. Animation uses transform only and stops under prefers-reduced-motion. There is no large video dependency. The screen recording supplied by the user was not accessible through the Windows recording path; timing was recovered from the live component instead.
