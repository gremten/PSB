...
CSS implements the 39-second loop using the live node positions, rotations and relative sizes. `public/figma/account/background-blob.svg` preserves the supplied path, radial gradient and 90 px blur; its canvas has 180 px of transparent padding on every side so Telegram WebView cannot clip the blur into a rotated rectangle. CSS compensates for that padding, so the visible glow keeps the live component's position and size without a raster/video dependency. Motion stops under prefers-reduced-motion. Timing was recovered from the live component.
