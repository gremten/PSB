...
| Cashback connected | Screen `2125:27598`; new-period live variant `2072:24153`; annual chart `1923:15118` | Current/annual segment, next-month state and category entry; annual chart keeps the existing November–April values and extends through December with deterministic fake values, native horizontal scrolling, and scroll-linked column scale (0.88–1) based on visible width. This is presentation-only; no product-state or replay contract changes. | existing `public/figma/home/banner-new-bg.svg` visual-affair asset | `cashback.period.*`, `cashback.next_month.*`, `cashback.year_chart.scroll` |
