...
| Card route + card-flip pieces | Flip components `2224:7924`, `2224:7960`, `2224:7931`, `2224:7965` | Cards follow a horizontal drag and switch only after the 168 px halfway threshold; shorter drags snap back. Tap/keyboard flips, fake detail copy and hide details remain unchanged. | `public/figma/card/**` | `card.*`, `navigation.back` |
...
- Participant demo feedback and the card-copy confirmation use the additive `GlassToast` component (`src/features/usability/glass-toast.tsx`), based on the existing `@samasante/liquid-glass` material. Owner surfaces: participant shell, `/card`, moderator live dashboard, and moderator replay; reference for the card-copy badge is Figma node `2072:14794`, while demo feedback has no claimed exact live Figma node. All auto-dismiss after three seconds idle, animate in/out over 200 ms, and follow a swipe toward their nearest viewport edge with proportional scaling. Dismiss controls use `demo.unavailable.toast.dismiss`, `card.copy.toast.dismiss`, `moderator.live.demo_toast.dismiss`, and `moderator.replay.demo_toast.dismiss`. The live dashboard shows a toast only for newly observed missclick events in the selected session. No new banking state or assets are introduced.
