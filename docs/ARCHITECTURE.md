...
| Category selection | Live connecting screen `2072:16481` | Up to three choices, invalid confirmation feedback, current/next-month branch; fixed single-action bottom bar, small card scroll parallax, exact account SVG geometry/filter and 39 s loop recolored with the category purple stops | `public/figma/categories/**` | `cashback.category.*`, `cashback.categories.*`, `cashback.next_month.*` |
