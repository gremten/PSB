...
Participant route modules and the static assets needed by the three flows are prefetched and decoded asynchronously while the participant waits for or receives a scenario. The shared bank stylesheet is owned by the persistent participant shell so route changes do not briefly repaint cached images against late route-local styles. Each participant route receives a 160 ms opacity settle after a pathname change, preventing text and CSS surfaces from flashing independently of already-decoded images without creating a containing block that would detach fixed action bars from the viewport; reduced-motion clients receive an immediate swap. Prefetching is refreshed when a new scenario is assigned. Session completion performs a one-time hard navigation to discard Next's in-memory Router Cache; immutable public Figma assets may remain in the browser's ordinary HTTP cache because they contain no participant or research data. Replay uses the same preloader inside its iframe. The replay detail reads the full ordered event stream rather than the bounded live-dashboard snapshot, runs only at 1×, paints its clock at animation-frame cadence, interpolates recorded scroll samples, restores copy toast/current-month/next-month success-sheet visibility from durable events, and visually separates task start and task finish boundaries. Replay retains real event order but gives rapid taps 420 ms, visible state changes 260 ms, screen views 320 ms, and scenario boundaries 600 ms of minimum dwell; dense scroll samples remain at their recorded cadence, while long idle gaps remain capped. On a terminal scenario action the participant shell immediately locks further instrumentation and replaces the product UI with a saving state; the event response carries authoritative scenario status, so the next gate appears without waiting for the two-second presence poll. A failed completion request releases the lock and returns to the active task.

Session JSON export is versioned as `psb.usability.starl.v3`. It remains backward-readable by retaining the raw `session`, `taskRuns`, and ordered `events`, while adding scenario coverage and one evidence-backed STARL record per task run. Situation and Task carry build/context and the approved participant prompt/success condition; Action carries chronological semantic events, an ordered Russian interaction narrative, elapsed milliseconds from that scenario's start, verdicts, and counts; Result carries completion, scenario duration, journey segment, and event-sequence confirmation; Learning deliberately contains cited evidence signals rather than an invented conclusion. An embedded Russian analysis prompt explains the three research scenarios, verdict semantics, STARL evidence rules, timing boundaries, entry-point analysis, and privacy limits for direct AI automation. `home.savings.open` inside card-copy is explicitly an off-path error because the savings account exposes no card data. Session timestamps remain audit context, but total session elapsed time is not an analytical field: waiting for assignment, moderator discussion, and time before `Старт` must never enter timing aggregates. Downstream automation must use only task-run timing, distinguish observation, inference, and recommendation, and cite event IDs. CSV retains its original first eight columns and appends schema, participant/build, scenario, task timing, semantic ID/label, verdict, and interpretation columns. A third `format=md` export is a UTF-8, Russian, human-readable report: it summarizes coverage and task-only timing, explains verdicts, calculates the participant's completion/journey/entry percentages, ranks frequent error and exploration actions, includes the dashboard's anonymized cross-session journey and product metrics with explicit denominators, presents each scenario's screen path and tap chronology in plain language with event citations, and appends the STARL analysis prompt without exposing raw event scripts. Exports use an unbounded ordered event query so they are never silently truncated; live moderator snapshots remain bounded. `participantCode` is the study pseudonym. Metadata has already passed the ingestion privacy sanitizer, and exports must not introduce banking values, clipboard contents, or other personal data.
