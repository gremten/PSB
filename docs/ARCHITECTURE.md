...
|---|---|---|
| `/` | Participant home | Main bank overview, horizontal promos, the current-account card plus add-product action, hide accounts, quick actions, collapsible history/currency, tabbar. The former savings-account card is not part of the current product architecture. |
...
|---|---|---|---|---|
| Home route + `ProfileHeader` + `Tabbar` | Header `825:4490`; toolbar `1488:4615`; promo `2072:15897`; hidden-account reference `2125:27356`; live UI Kit tabbar `1338:4518` | Promo dismissal, horizontal scrolling with only the current account and add-product action, account hiding, block collapse, currency segment, tab demo guards; header toolbar uses optical glass; tabbar's five 24 px icon silhouettes follow the live UI Kit. The savings-account card has been removed by explicit product decision. | `public/figma/home/**` | `home.*`, `header.*`, `tab.*`, `cashback.tab.open` |
...
A participant who enters a pseudonym creates one **pending**, unrecorded session. The moderator chooses a scenario for that session; the participant sees its text prompt and presses `Старт`. Only then does `started_at` become non-null, a `task_run` begin, and events acquire that run ID. Between flows, the participant returns to a waiting gate and no product interaction is recorded. After each successful flow, the same research gate asks the standard Single Ease Question (SEQ), 1–7, before exposing the next assignment or final completion state. Its seven additive controls use stable `participant.seq.score.1` … `.7` IDs, write only `task_runs.ease_score`, contain no banking/product state, and survive the final-flow cache clear through a session-scoped pending marker. The only unrecorded-demo escape is the subdued top-right `Без сессии` control on the original name gate; it creates no database session. Waiting, assigned, completed, and ended scenario gates do not expose that control. The name gate submits through the orange `Начать тест` primary action. The gate is an additive component owned by `ParticipantShell`, with no Figma node (user-supplied text-only brief), local `scenario-gate.module.css`, stable `participant.session.create/skip` and `participant.scenario.start` IDs, and no new banking state or assets.
...
- While a task run is actively recording, the moderator UI does not mount or offer replay for that session. Metrics remain readable, and replay becomes available immediately after the active run ends.
...
Each scenario also reports standard task-level measures: total/unaided/aided completion among unique starters; failure; error-free completion and direct-path completion among finishers; first-click success among attempts with a tap; median and interpolated P75 completion time among successful runs; median and P75 taps above the approved golden tap count; SEQ mean/median, response count, and share scoring 5–7; and failed-run stage drop-offs. Binary participant rates carry Wilson 95% confidence intervals. Active attempts and corrupted data-quality exclusions are disclosed separately rather than silently changing a denominator. A direct path is an unaided completion with no error, recovery, informational exploration, or excess tap. Problem prevalence groups error events by scenario and semantic control, but the numerator is unique affected participants and the denominator is unique participants who started that scenario; raw repeat count is supporting context only. It also reports the descriptive completion split for affected versus unaffected participants and recovery among affected participants, explicitly without a causal claim. The participant list provides direct MD download and confirmed deletion without opening replay detail.

Participant route modules and the static assets needed by the three flows are prefetched and decoded asynchronously while the participant waits for or receives a scenario. The shared bank stylesheet is owned by the persistent participant shell so route changes do not briefly repaint cached images against late route-local styles. Each participant route receives only a 90 ms, 0.96→1 opacity settle after a pathname change, preventing text and CSS surfaces from flashing independently of already-decoded images while keeping perceived navigation latency minimal; reduced-motion clients receive an immediate swap. Prefetching is refreshed when a new scenario is assigned. Session completion performs a one-time hard navigation to discard Next's in-memory Router Cache; immutable public Figma assets may remain in the browser's ordinary HTTP cache because they contain no participant or research data. Replay uses the same preloader inside its iframe. The replay detail reads the full ordered event stream rather than the bounded live-dashboard snapshot, runs only at 1×, paints its clock at animation-frame cadence, interpolates recorded scroll samples, restores copy toast/current-month/next-month success-sheet visibility from durable events, and visually separates task start and task finish boundaries. Replay retains real event order but gives rapid taps 420 ms, visible state changes 260 ms, screen views 320 ms, and scenario boundaries 600 ms of minimum dwell; dense scroll samples remain at their recorded cadence, while long idle gaps remain capped. On a terminal scenario action the participant shell immediately locks further instrumentation and replaces the product UI with a saving state; the event response carries authoritative scenario status, so the SEQ gate appears without waiting for presence polling. A failed completion request releases the lock and returns to the active task. Scenario start boundaries carry the participant capture timestamp and every event carries its active scenario code. A delayed request captured before the new task is rejected instead of being attached to that run; replay, first-click analysis, task duration and export chronology use the same validated participant capture clock with a server-time fallback. A narrowly scoped ten-second grace window preserves only a late-arriving success-sheet close/drag tap in its just-completed scenario. Event ingestion avoids publishing unchanged legacy control state, skips scenario-state reconstruction for scroll/screen-view events, completion reuses the already loaded run, participant assignment polling is 750 ms, and the live moderator refresh is 1500 ms.

Session JSON export is versioned as `psb.usability.starl.v5`. It remains backward-readable by retaining the raw `session`, `taskRuns`, and ordered `events`, while adding scenario coverage and one evidence-backed STARL record per task run. Situation and Task carry build/context and the approved participant prompt/success condition; Action carries chronological semantic events, an ordered Russian interaction narrative, elapsed milliseconds from that scenario's start, verdicts, and counts; Result carries completion, participant-capture duration with server fallback, first-click correctness, error-free/direct-path flags, excess taps, SEQ, journey segment, and event-sequence confirmation; Learning deliberately contains cited evidence signals rather than an invented conclusion. An embedded Russian analysis prompt explains the three research scenarios, verdict semantics, STARL evidence rules, timing boundaries, entry-point analysis, metric denominators, confidence intervals, descriptive-versus-causal limits, and privacy limits for direct AI automation. Historical `home.savings.open` events remain readable and are labelled as interactions from an older interface version; the current interface cannot emit them. Session timestamps remain audit context, but total session elapsed time is not an analytical field: waiting for assignment, moderator discussion, and time before `Старт` must never enter timing aggregates. Downstream automation must use only task-run timing, distinguish observation, inference, and recommendation, use unique participants for issue prevalence, report uncertainty, and cite event IDs. CSV retains its original first eight columns and appends schema, participant/build, scenario, task timing, semantic ID/label, verdict, and interpretation columns. A third `format=md` export is a UTF-8, Russian, human-readable report: it summarizes coverage and task-only timing, explains verdicts, calculates the participant's completion/journey/entry percentages, ranks frequent error and exploration actions, includes the dashboard's anonymized cross-session standard metrics, Wilson 95% confidence intervals, data-quality exclusions and descriptive issue impact with explicit denominators, presents each scenario's screen path and tap chronology in plain language with event citations, and appends the STARL analysis prompt without exposing raw event scripts. Exports use an unbounded ordered event query so they are never silently truncated; live moderator snapshots remain bounded. `participantCode` is the study pseudonym. Metadata has already passed the ingestion privacy sanitizer, and exports must not introduce banking values, clipboard contents, or other personal data.
...
3. The three interactive flows use automatic task start/finish and expose completion, correct/error/recovery tap counts, timing, per-flow replay seek points, and a participant-reported SEQ 1–7 after successful completion. Historical A/B ease-score controls remain unexposed; do not fabricate moderator assistance or ratings when no SEQ response exists.
