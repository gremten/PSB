...
The shared `BankHeader` in `src/features/bank/bank-ui.tsx` owns the sticky progressive-blur substrate and exposes the existing `ProfileHeader` and `DetailHeader` variants without changing their public props or tracking. Its web layer is always 69 px high: the 61 px header plus an 8 px fade tail, anchored below any Telegram safe-top inset so the native chrome is not blurred. Figma header `825:4490` includes a native status area in its 123 px blur layer, which Telegram draws outside the web header. Home, account, and category selection draw under Telegram's fullscreen chrome using the existing safe-top variable; category selection alone adds the account-style page-colored native header to avoid a gray strip. Both main cashback states use the same transparent blur header over the `#161a20` top surface from disconnected frame `2072:16263`; neither adds an opaque header substrate. No route may add another 123 px header pseudo-layer.
