...
| Cashback connected | Screen `2125:27598`; new-period live variant `2072:24153`; annual chart `1923:15118` | Current/annual segment, next-month state and category entry; annual chart shows a May–April historical year, with April at the right edge and older months only to its left. May–October have deterministic fake values. The chart starts at its rightmost scroll position and keeps native horizontal scrolling with scroll-linked column scale (0.88–1) based on visible width. This is presentation-only; no product-state or replay contract changes. | existing `public/figma/home/banner-new-bg.svg` visual-affair asset | `cashback.period.*`, `cashback.next_month.*`, `cashback.year_chart.scroll` |
| Category selection | Live connecting screen `2072:16481` | Up to three choices, invalid confirmation feedback, current/next-month branch; fixed single-action bottom bar with 32 px top corners and 46 px button bottom spacing, small card scroll parallax, exact account SVG geometry/filter and 39 s loop recolored with the category purple stops; Telegram fullscreen header/stage use page color and account-style safe-top layout | `public/figma/categories/**` | `cashback.category.*`, `cashback.categories.*`, `cashback.next_month.*` |
