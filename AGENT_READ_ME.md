...
- The cashback category-selection screen follows live node `2072:16481`: reuse the account's 39 s background-position/rotation/size sequence with the local purple Figma SVG; the hero uses the exact local card export with a subtle scroll-linked vertical offset. The 24 px round FAQ glyphs and checkbox geometry remain within the existing category-row tap target. The bottom control is a fixed single-action bar, not the global navigation tabbar; preserve its `cashback.categories.confirm` tracking and selection/confirmation logic.
