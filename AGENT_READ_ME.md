...
- The participant header is one shared `BankHeader` with profile and detail variants. Its 69 px progressive blur is anchored to the bottom of the web header, below Telegram's safe-top inset; Figma's 123 px layer already includes the native status area and must not be duplicated below Telegram chrome. Cashback keeps its `#161a20` top page surface but must not add an opaque gray header substrate: the shared blur remains translucent when content scrolls behind it. The bento starts at its existing 68 px position. Do not add route-specific header blur pseudo-elements.
