...
- The moderator JSON download is the canonical machine-readable AI-analysis artifact (`psb.usability.starl.v3`): preserve raw `session`/`taskRuns`/`events`, STARL evidence records, the embedded Russian analysis prompt, ordered interaction narratives, schema version, event citations, and the observation/inference/recommendation boundary. CSV is a flat event interchange and must retain its legacy first eight columns. The adjacent Markdown download is the human-readable Russian report: keep its scenario summaries, task-only timing, participant completion/journey/entry percentages, ranked error and exploration actions, anonymized cross-session dashboard metrics with explicit denominators, plain-language chronology, event citations, verdict explanation, and appended STARL prompt. Never silently cap export events; moderator live views may remain bounded.
