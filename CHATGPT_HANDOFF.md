...
- The moderator JSON download is the canonical machine-readable AI-analysis artifact (`psb.usability.starl.v5`): preserve raw `session`/`taskRuns`/`events`, STARL evidence records, the embedded Russian analysis prompt, participant-capture ordering with server fallback, ordered interaction narratives, schema version, event citations, first-click/error-free/direct-path/excess-tap/SEQ results, confidence intervals, data-quality exclusions, descriptive issue impact, and the observation/inference/recommendation boundary. CSV is a flat event interchange and must retain its legacy first eight columns. The adjacent Markdown download is the human-readable Russian report: keep its scenario summaries, task-only timing, participant completion/journey/entry percentages, standard per-scenario metrics, unique-participant issue prevalence, ranked error and exploration actions, anonymized cross-session dashboard metrics with explicit denominators, plain-language chronology, event citations, verdict explanation, and appended STARL prompt. Never silently cap export events; moderator live views may remain bounded.
