...
- Entering a pseudonym through the orange `Начать тест` action creates an unrecorded pending session. Only the subdued top-right `Без сессии` control on that first screen enters the unrecorded demo; scenario waiting/assignment/end gates have no such escape. Only `Старт` after moderator assignment starts a task run and recording. Card copy finishes when its upper toast disappears; first and next-month cashback finish on a valid sheet dismissal. The client locks instrumentation on that terminal action and uses the event response for the next gate instead of waiting for polling. The third flow is unavailable until first cashback connection; all three completions end the session. Preserve server-classified `scenarioVerdict` on taps: correct/recovery is green, errors red, and card-copy exploration of the savings account, category help, or Cashback month/year exploration is yellow `info` rather than a missclick. Scroll is not an error.
