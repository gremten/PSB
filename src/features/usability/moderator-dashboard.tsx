...
export function ModeratorDashboard({ initialSessions, initialSnapshot, initialMetrics }: Props) {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    <div className={styles.dashboardGrid}>
        ...
        {summaryOpen && (summaryLoading ? <div className={styles.empty}>Считаем показатели…</div> : researchSummary ? <>
          ...
          <div className={styles.scenarioSummaryGrid}>{sortedScenarioMetrics.map((scenario) => <article className={styles.scenarioSummary} key={scenario.code}>
            ...
            <div className={styles.metricGrid}>
              ...
              <div className={styles.stat}><span>Чистый маршрут</span><strong>{scenario.directPathRate}%</strong><small className={styles.summaryDenominator}>без ошибок, изучения и лишних тапов · {confidenceLabel(scenario.directPathConfidence95)}</small></div>
      ...
      {initialMetrics.length > 0 && <section className={styles.card}><h2>Агрегаты завершённых задач</h2><div className={styles.sessionTableWrap}><table className={styles.metrics}><thead><tr><th>Task</th><th>Completion</th><th>Unaided</th><th>Error-free</th><th>Чистый маршрут</th><th>First click</th><th>Median / P75 time</th><th>Median / P75 extra taps</th><th>SEQ</th></tr></thead><tbody>{initialMetrics.map((metric) => <tr key={metric.taskCode}><td>{metric.taskCode}</td><td>{Math.round(metric.completionRate * 100)}%</td><td>{Math.round(metric.unaidedCompletionRate * 100)}%</td><td>{Math.round(metric.errorFreeCompletionRate * 100)}%</td><td>{Math.round(metric.directPathRate * 100)}%</td><td>{Math.round(metric.firstClickSuccessRate * 100)}%</td><td>{metric.medianCompletionTimeMs === null ? "—" : duration(metric.medianCompletionTimeMs)} / {metric.p75CompletionTimeMs === null ? "—" : duration(metric.p75CompletionTimeMs)}</td><td>{metric.medianExcessTaps ?? "—"} / {metric.p75ExcessTaps ?? "—"}</td><td>{metric.easeMedian ?? "—"}/7 ({metric.easeResponseCount})</td></tr>)}</tbody></table></div></section>}
    </div>
