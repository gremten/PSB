...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as
    | { eventName?: string; sessionId?: string; scenarioCode?: string | null; screen?: string; action?: string; target?: string; metadata?: Record<string, unknown> }
    | null;
  ...
  const event = await recordParticipantEvent({
    ...
    scenarioCode: Object.prototype.hasOwnProperty.call(body, "scenarioCode")
      ? typeof body.scenarioCode === "string" ? body.scenarioCode : null
      : undefined,
  ...
  const status = event && body.sessionId && completionScenario ? await getParticipantScenarioStatus(body.sessionId, false) : null;
