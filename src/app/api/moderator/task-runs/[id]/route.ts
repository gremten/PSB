...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    finishTask(id, { result: body.result, easeScore: body.easeScore!, easeReason: body.easeReason, moderatorNote: body.moderatorNote, corruptedReason: body.corruptedReason });
    return NextResponse.json({ snapshot: getSessionSnapshot(existing.sessionId), research: getResearchState(), metrics: getAggregateMetrics() });
